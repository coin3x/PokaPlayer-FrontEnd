      ...
      <div class="item" @click="showSoundQualityDialog = true" v-ripple>
        <div class="content">
          ...
          <div class="header">
            ...
            <div
              class="t-ellipsis"
            >{{ $t(`settings.network.soundQuality.${currentSoundQualityLocalizationKey}.title`) }}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  name: 'SettingNetwork',
  data: () => ({
    soundQuality: window._setting('audioQuality').toLocaleLowercase(),
  ...
  computed: {
    currentSoundQualityLocalizationKey() {
      let option = this.soundQualityOptions.find(opt => opt.settingValue === this.soundQuality)
